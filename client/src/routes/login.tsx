...
function RouteComponent() {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      ...
      // Adjusted: Extract the user data from the response
      // This assumes your backend sends the user in response.data.data.user
      const user = response.data.data.user;
      login(user); // Store the user data in your auth store
      navigate({ to: "/" });
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen">
      <div className="bg-primary text-background w-[150px] h-[100px]">LOGO</div>
      <div className="w-[450px] rounded-md py-5 bg-white mx-auto mt-2">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-12 w-[80%] mx-auto">
          <div className="flex flex-col gap-y-4">
            <div id="emailBody">
              <Label htmlFor="email">Email</Label>
              <Input type="text" {...register("email")} placeholder="xyz@mail.com" />
              {errors.email && (
